...
// similarity-ignore: vendor した anti-slop（ADR 0009）の upstream コード。
// 2 つのルールファイルが同じヘルパを持つのは upstream の設計であり、
// 本テンプレートが重複解消の責任を負う対象ではない。再 vendor 時は
// このコメントを付け直すこと（docs/recipes/tools/typescript.md 参照）。
