...
import { GraphLocatorError, resolveCanonicalRepositoryRoot } from "../src/locator.ts";
import { createCanonicalRepo, createLinkedWorktree } from "./support/fake-repo.ts";

// 設計正本 §9 / §10（hard gate 2）の「hook はどの worktree の cwd からでも
// canonical graph の locator を解決する」のうち、位置の解決部分を検証する。
// セッション cwd（worktree のルートやその子ディレクトリもありうる）から
// canonical リポジトリのルートを返せること、解決できない場合は
// GraphLocatorError で fail-closed になることを契約として固定する。
//
// eslint/max-lines-per-function は関数の物理的な行範囲を見るため、可変データは
// モジュールスコープに置き、describe は兄弟に分けている。

/** `.git` ファイルの中身（1行目）と worktree ルートから壊れた配置を作るケース。 */
const BROKEN_GIT_FILE_CASES = [
  ...
  },
  {
    ...
    // submodule 形（<main>/.git/modules/<name>）を指すケース。ramune の対象外の
    // 配置であり、誤って main リポジトリへ解決しないよう拒否する。
    writeGitFile: (worktreeRoot: string, parentDir: string): void => {
...
describe(resolveCanonicalRepositoryRoot, () => {
  let parentDir: string;

  beforeEach(() => {
    parentDir = fs.mkdtempSync(path.join(os.tmpdir(), "ramune-hooks-locator-test-"));
  });
  ...
  it("canonical リポジトリの子ディレクトリを渡すと、親方向に辿ってルートを返す", () => {
    expect.hasAssertions();
    const repositoryRoot = createCanonicalRepo(parentDir);
    const subDirectory = path.join(repositoryRoot, "apps", "web");
...
describe("resolveCanonicalRepositoryRoot: linked worktree の cwd から canonical リポジトリを返す", () => {
  ...
  it("linked worktree の子ディレクトリを渡しても、canonical リポジトリのルートを返す", () => {
    ...
    const repositoryRoot = createCanonicalRepo(parentDir);
    const worktreeRoot = createLinkedWorktree(repositoryRoot);
    const subDirectory = path.join(worktreeRoot, "tools", "ramune");
    fs.mkdirSync(subDirectory, { recursive: true });
    expect(resolveCanonicalRepositoryRoot(subDirectory)).toBe(repositoryRoot);
  });
  ...
  it(".git ファイルの gitdir が相対パスで書かれていても解決できる", () => {
    expect.hasAssertions();
    const repositoryRoot = createCanonicalRepo(parentDir);
    // 実物の git は環境によって gitdir を相対パスで書く。worktree ルートからの
    // 相対指定に書き換えた .git ファイルでも同じく解決できること。
